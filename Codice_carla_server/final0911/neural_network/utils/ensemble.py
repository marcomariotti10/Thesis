...
def visualize_prediction(pred, gt, map, pred1, pred2):
    fig, ax = plt.subplots(3, 2, figsize=(10, 10))
    print(pred1[0][0], pred2[0][0], pred1[200][200], pred2[200][200])
    print(map[0][0], gt[0][0], map[200][200], gt[200][200])
    print(pred[0][0], pred[200][200])
    ...
    ax[1,1].set_title('Overlay of Original and Prediction Grid Maps 2')
     ...
     # Calculate and visualize the differences
    diff1 = np.abs(pred - pred1)
    diff2 = np.abs(pred - pred2)

    ax[2,0].imshow(diff1, cmap='hot')
    ax[2,0].set_title('Difference between Prediction and Prediction 1')

    ax[2,1].imshow(diff2, cmap='hot')
    ax[2,1].set_title('Difference between Prediction and Prediction 2')

    plt.show()

def model_preparation(model_name_1, model_name_2):
    ...
    return model_1, model_2, device

def evaluate(model_1, model_2, device):
    test_losses = []
...
def show_predictions(model_1, model_2, device):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                for i in range(len(grid_maps)):
                    visualize_prediction(predictions[i], gt[i], grid_maps[i], predictions1[i], predictions2[i])
...
if __name__ == '__main__':
    ...
    model_name_1 = 'model_20250301_023205_loss_0.0048_weightedloss'
    model_name_2 = 'model_20250301_181025_loss_0.0020_BCEwithlogitloss'

    model1, model2, device = model_preparation(model_name_1, model_name_2)

    #evaluate(model1, model2, device)

    show_predictions(model1, model2, device)
